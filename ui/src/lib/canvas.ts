 ...
 *
 * `.canvas` 存 Open LLM Wiki 自有 JSON schema(引擎 = Excalidraw),**不**依赖
 ...
 */
export interface OpenLlmWikiCanvas {
  openLlmWikiCanvas: typeof CANVAS_SCHEMA_VERSION;
...
export function isLegacyTldrawCanvas(raw: string): boolean {
  ...
  // 新 schema 明确带版本标记,不算 legacy(兼容旧键 openobsidianCanvas)。
  if (o.openLlmWikiCanvas != null || o.openobsidianCanvas != null) return false;
...
export function parseCanvasContent(
  ...
  const o = parsed as Record<string, unknown>;
  const schemaVer = o.openLlmWikiCanvas ?? o.openobsidianCanvas;
  if (schemaVer !== CANVAS_SCHEMA_VERSION) return null;
...
/** 序列化为美化 JSON(git 友好)。 */
export function serializeCanvasContent(doc: OpenLlmWikiCanvas): string {
