...
> 原则:复用 `open-llm-wiki-core` 语义;前端 mock 可跑;桌面/agent 真路径可接。
...
mcp/                 ← Rust bin: open-llm-wiki-core + vault fs, MCP tools(含 `run_qql`,agent 用)
ui/src/lib/plugin-*  ← 宿主 API / 加载 vault `.open-llm-wiki/plugins/`
...
2. MCP:`cargo run -p open-llm-wiki-mcp -- /path/to/vault` + tools/list 非空;read/write 真文件。  
