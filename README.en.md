...
- **AI-ready (MCP):** a built-in MCP server exposes 8 tools (`list_notes` / `read_note` / `write_note` / `links` / `search_notes` / `run_qql` / `vault_info` / `lint_vault`) so agents like Claude Desktop can read and write your vault; `lint_vault` runs structural lint and yields candidates only, never verdicts.
...
- `mcp/` — the built-in MCP server (8 tools) for AI agents.
