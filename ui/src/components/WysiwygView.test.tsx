 ...
 *
 * BlockNote 真实渲染 + md↔blocks round-trip 由 e2e 覆盖(见 smoke.spec.ts)。
 *
...
describe("WysiwygView", () => {
  it("hasNote=false 时显示空态", () => {
    render(
      <WysiwygView content={PLAIN} onChange={() => {}} hasNote={false} theme="dark" t={t} />,
    );
  ...
  it("挂载时把 body(已剥离 frontmatter)解析为块载入 editor", () => {
    ...
    render(<WysiwygView content={HEAD} onChange={() => {}} hasNote={true} theme="light" t={t} />);
  ...
  it("BlockNote 变化后防抖回写,合并最新 fm + 新 body", () => {
    ...
    render(<WysiwygView content={OLD} onChange={onChange} hasNote={true} theme="dark" t={t} />);
  ...
  it("卸载时 flush 未触发的编辑(不丢输入)", () => {
    ...
    const { unmount } = render(
      <WysiwygView content={PLAIN} onChange={onChange} hasNote={true} theme="dark" t={t} />,
    );
    act(() => fireEvent.click(screen.getByTestId("bn-mock")));
