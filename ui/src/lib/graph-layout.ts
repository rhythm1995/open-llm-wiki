 ...
 *
 * 力导向排布现已交给 react-force-graph-2d 内部的 d3-force(见 graph-d3-forces),
 * 本文件只保留渲染器中立的共享件:
 *   - Pt / ForceParams / DEFAULT_FORCES / normalizeForces —— 力参数模型
 *   - bbox / fitTransform / visibleNodeIds —— 坐标几何(被测试覆盖;旧 SVG 层用,
 *     现 GraphForceLayer 自带 zoomToFit,这些留作通用工具)。
 *
 * 无 IO、无 React、可单测。
 */
export interface Pt {
 ...
 * 被 graph-d3-forces 映射成 d3-force 的 strength/distance。
 */
